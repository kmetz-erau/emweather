...
export default function OperationsBriefingPanel({ weather, alerts }: { weather: WeatherData[]; alerts: Array<AlertsData | null> }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/ai', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mode: 'briefing', weather, alerts }) })
  ...
  async function ask() {
    ...
    try {
      const r = await fetch('/api/ai', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mode: 'ask', question, weather, alerts }) });
  ...
  return <div className={`border rounded-lg overflow-hidden ${statusColor}`}>
    <div className="px-4 py-3 border-b border-[#30363d] flex justify-between gap-3">
      ...
      {briefing && <span className="text-[10px] uppercase font-mono text-slate-500">{briefing.generatedBy === 'ai' ? 'AI assisted' : 'Rules mode'} · {briefing.confidence} confidence</span>}
    </div>
    <div className="p-4">
